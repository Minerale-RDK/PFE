...
def home():
    ...
    print(f'Alarmes de sum_gene = {sum_gene}, alarmes de sum_client {sum_client}')

    return render_template("index_rescue.html", client=client, generateur=generateur, mat = matrice, conso=conso, prod=prod, 
...
cert_idx = 1
cert = f"/certificates-all/certificate-scada-rescue-{cert_idx}.der"
private_key = f"private-key-scada-rescue-{cert_idx}.pem"


def testStatusClient():

    ##-W <sec> : Changer la durée d'attente de réponse DEFAULT=10 
    response = os.system("ping -c 1 -W 5 client-scada1 > /dev/null 2>&1")
...
class AlarmeHandler:
    ...
    async def datachange_notification(self, node: Node, val, data):
        ...
        index = int(self.url_gene.split('opc.tcp://server-gene')[1][:1]) - 1
        alarmesGene[index] = val   
